...
(`gtm/scripts/design_room_pulse.py`): a scheduled job evaluates new repos/trends
...
## Video hero (produced, not decoration)

The hero can be a *produced* demo video, generated in code from the same React/TS the
site is built in — versioned and re-renderable from parameters, not a one-off export.
Pick ONE engine per project.

| Repo | Install | Unlocks | License |
|---|---|---|---|
| [remotion-dev/remotion](https://github.com/remotion-dev/remotion) | `npm i remotion` | Video authored as React components; `renderMedia` + `inputProps` parametrize each output | free for individuals + companies ≤3 employees; paid at 4+ (remotion.pro). Free at our size (company of 1). |
| [motion-canvas/motion-canvas](https://github.com/motion-canvas/motion-canvas) | `npm init @motion-canvas` | Timeline-based code-driven explainer video | MIT |
| [redotvideo/revideo](https://github.com/redotvideo/revideo) | `npm i @revideo/core` | Headless render API for programmatic/parametrized video, zero-dep | MIT |

Selection: Remotion when the hero is React UI in motion (free at our size). Motion Canvas
or Revideo (both MIT, free forever) for code-driven explainer video, or as the escape
hatch if we ever outgrow Remotion's free tier.
