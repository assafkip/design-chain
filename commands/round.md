...
here = pathlib.Path(os.environ.get("CLAUDE_PROJECT_DIR") or os.getcwd()).resolve()
for d in [here, *here.parents]:
    c = d / "design-chain.json"
    if c.is_file():
        try:
...
UNREADABLE or NO: stop and say so.
...
that the page is good. The gate measures floors; your eye and real buyer conversations are the bar.
...
folder the config maps (`round_lane()`); the lane decides which web-only stages apply.
...
The readers. `design-reader-gate.py` runs the buyer persona against the served page and appends rows
