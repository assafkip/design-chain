...
Run: python3 hooks/test_dogfood_gate.py
"""
import sys, os
...
# ── scar-134: the public-vs-internal path classifier is the DETERMINISTIC half of
...
INTERNAL_PATHS = [
    "/repo/internal/output/daily-schedule-2026-07-31.html",
    "/repo/internal/marketing/templates/post.html",
    ...
    "/repo/internal/output/morning-log-view.html",
]
PUBLIC_PATHS = [
...
ok("[scope] uppercase internal path still skipped",
   is_public_facing_page("/repo/Internal/Output/Schedule.HTML") is False)
ok("[scope] uppercase public page still scanned",
   is_public_facing_page("/repo/sites/eyeball/INDEX.HTML") is True)

# ── scar-134 regression, Codex major on PR #49: the classifier called the founder-only
...
COCKPIT_REAL_PATHS = [
    "/home/someone/projects/gtm/gtm/cockpit/index.html",
    "/home/someone/projects/gtm/gtm/cockpit/content.html",
]
...
if failures:
    ...
    for f in failures:
        print("  - " + f)
    sys.exit(1)
# ── scar-511: two false positives that blocked five legitimate site edits ──
#
# The gate exit-2'd every edit to the reference site on 2026-08-08. A gate that
...
print("test_dogfood_gate: scar-511 regressions checked")
