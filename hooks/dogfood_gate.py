...
BRAND_FILES = ("marketing/brand-kit.html", ".design-brand.json", "brand-kit.html")


def brand_colors_for(path):
    """Colours the project has DECLARED as its own, walking up from `path`.

    why this exists (scar-511, 2026-08-08): the palette tells catch a hue family
    that is the current AI default -- amber/orange among them. The reference site's
    documented primary accent is a copper that lives in that family, so the gate
    flagged every one of its pages for using the brand on purpose. The
...
def scan_html(content, fp, brand=None):
    ...
    if palette:
        ...
        # why the split (scar-511, 2026-08-08): this scanned EVERY colour literal
        # in the stylesheet and matched them against every palette token,
        # including "warm-cream-bg". The reference site is a near-black page whose
    ...
    # why anchors count (scar-511, 2026-08-08): this demanded a form, input or
...
# to be public.
#
# "/cockpit/" is here because the classifier called an internal GTM cockpit
# page PUBLIC, and that page is
...
# positive (scar-134, Codex major on PR #49). Measured at the time of the fix: 465
...
INTERNAL_PATH_MARKERS = (
    "/internal/", "/node_modules/", "/templates/", "/template/", "/test", "/tests/",
...
def is_public_facing_page(path):
    ...
    (scar-134). One chokepoint now: this hook's main() and test_dogfood_gate.py both
...
def check_cli(argv):
    ...
    why (scar-1746, dc-11): the hook exits 0 for a clean page AND for one it did not look at
