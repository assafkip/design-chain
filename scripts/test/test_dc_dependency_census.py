...
Scar, 2026-09-18 (scar-1796): a census that read static imports only reported "stdlib plus
...
# dependency in the set is node, for the impeccable detector vendored under vendor/impeccable
# (resolved from the script's own location, so it is not an external path).
DECLARED_EXECUTABLES = {"node", "git", "claude"}   # claude: the reader gate's model runner (dc-06)
DECLARED_EXTERNAL_PATHS = {
    "~/.config/design-chain",      # the gate's per-session ledger directory
}
