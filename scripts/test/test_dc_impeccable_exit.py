...
REAL_DETECTOR = SCRIPTS.parent / "vendor" / "impeccable" / "detector" / "detect-antipatterns.mjs"
...
class EveryRecordedStageIsBelievable(unittest.TestCase):
    """scar-1845 happened because dc-04 recorded a stage the receipt check (dc-10) had no script for.
...
class SealRunsIt(unittest.TestCase):
    ...
    def setUp(self):
        ...
        if not REAL_DETECTOR.is_file():
            self.fail(f"the vendored impeccable detector is missing ({REAL_DETECTOR})")
        if not shutil.which("node"):
    ...
    def test_a_round_sealed_with_impeccable_reads_sealed_afterwards(self):
        # scar-1845: the impeccable stage had no producer on record for dc-10's receipt check, so a
