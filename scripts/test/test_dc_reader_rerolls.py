#!/usr/bin/env python3
"""scar-1840: seal counts a page's reader runs over every version of its bytes, caps them per round,
...
Cross-round and cross-commit re-rolls are scar-1834's (CI recompute), not tested here.
"""
import json
import unittest
...
class Rerolls(Runs):
    ...
    def test_renaming_the_page_does_not_reset_its_history(self):
        # scar-1840 adv-1: the count keyed on the page name, so a rename started it over
        self.config(reader_runs_max=1)
        self.read_page(*self.LEAVE)
    ...
    def test_raising_the_cap_after_the_runs_refuses(self):
        # scar-1840 adv-2: the cap was read live and never recorded
        self.config(reader_runs_max=1)
        self.read_page(*self.LEAVE)
        self.edit(1)
        self.read_page(*self.STAY)
    ...
    def test_rows_with_no_run_id_from_one_invocation_are_one_run(self):
        # scar-1840 std-2: one old n=5 invocation counted as five runs
        self.config(n=5)
    ...
    def test_the_cap_message_names_a_remedy_that_works(self):
        # scar-1840 std-1: it named editing the page and a FOUNDER line, neither of which lowers the count
        self.config(reader_runs_max=1)
        self.read_page(*self.LEAVE)
        self.edit(1)
        self.read_page(*self.STAY)
