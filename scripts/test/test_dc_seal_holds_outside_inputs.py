#!/usr/bin/env python3
"""scar-1831: seal reads the inputs outside the round from held bytes, not the live tree.

WHY. scar-1811 made the chain read every ROUND file from the snapshot. Everything it reads
...
each reproduced). A before/after compare lets X-then-back-to-A through (the scar-1808 scar),
...
class AnInputSwappedOnlyInsideTheWindowIsNotWhatSealReads(Base):
    ...
    def test_9_the_producers_get_the_held_references_and_config(self):
        ...
        # (producer, args) made every seal here a TypeError, red and unseen since (scar-1841)
