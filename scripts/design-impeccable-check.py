...
1. A KNOWN-SLOP CONTROL runs in the same invocation. Measured 2026-09-15: the first
   control chosen for this round sat under a `/output/` path, which the dogfood
   tripwire treats as internal, so it returned exit 0 and a SKIP read as a PASS
   (scar-1746). A clean report whose control never fired is decoration.
...
docs/LESSONS.md (a defect-absence gate is a floor, not a finish line). The craft
...
# The vendored detector (vendor/impeccable, Apache-2.0, see its NOTICE), resolved from this
# file so the check works from any cwd. --detector overrides it.
DETECTOR_CANDIDATES = (
    str(Path(__file__).resolve().parent.parent / "vendor" / "impeccable" / "detector" / "detect-antipatterns.mjs"),
)
...
# ---------------------------------------------------------------------------------------------
# THE ONLY TWO WAYS A FINDING STOPS COUNTING (round 2026-09-21, scar-1743). Both are decided
