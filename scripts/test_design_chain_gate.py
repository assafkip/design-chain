...
# dc-02 (scar-1796): seal RUNS the producers, which are the siblings of the gate file. The
...
class TestBlocks(Base):
    ...
    def test_internal_html_ignored(self):
        internal = self.inst / "internal" / "output" / "daily-schedule-2026-09-15.html"
...
class TestCraftBar(Base):
    ...
    docs/LESSONS.md (a defect-absence gate is a floor, not a finish line) already
    named (from an earlier design-room post-mortem, 2026-06-25). These
    ...
    def test_empty_craft_manifest_is_a_gap_not_a_pass(self):
        """Declaring nothing must not be the cheapest way to comply.
...
# scar-1824: the only main() call is at the END of this file. One sat here, mid-module, and
# `python3 <file>` exited before the 38 tests below it existed: every scar-1796 closeout said
...
class TestGapCheck(Base):
    ...
    # scar-1824: seal RUNS the gap producer (dc-02) and removes any prior gap.json first (dc-03),
...
class TestCorrectionsReachSearchText(TestCorrections):
    ...
    2026-09-16 auditing the reference site: two unverified numbers and a
    false claim sat in meta content the visible fix could not reach,
