#!/usr/bin/env python3
"""design-engine-door.py -- PreToolUse and PostToolUse on the Skill tool (dc-18, dc-19, scar-1796).
...
STATE_DIR = Path(os.environ.get("DESIGN_CHAIN_STATE", os.path.expanduser("~/.config/design-chain")))
...
def decide(payload: dict) -> tuple[int, str]:
    ...
    if os.environ.get("DESIGN_CHAIN_ALLOW") == "1":
        return 0, ""
    raw = str((payload.get("tool_input") or {}).get("skill") or "").strip()
    name = canonical(raw)                  # "design-plugin:brand" is the brand engine
    if not name or name not in listed():
        return 0, ""
