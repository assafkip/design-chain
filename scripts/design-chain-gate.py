...
# docs/LESSONS.md (a defect-absence gate is a floor, not a finish line) already named,
# distilled from an earlier design-room post-mortem (2026-06-25). Its four
...
#      naming a page (fixed 2026-09-15)
...
TRIPWIRE = str((HERE.parent / "hooks" / "dogfood_gate.py").resolve())
...
class RoundSnapshot:
    ...
    WHY (scar-1808): dc-03 compared the round before and after measuring. A -> X -> A walks
    ...
    scar-1831: the inputs OUTSIDE the round are held too. `root` is a private tree that mirrors
...
def _held_path(root: Path, p: Path) -> Path:
    ...
    not the resolved one, because the config is found by walking the typed path (scar-1811)."""
...
def _outside_inputs(root: Path, rd: Path, files: dict[str, bytes], cfg: dict, cfg_path: Path | None,
    ...
    the seal (scar-1833; holding without binding let a file swapped just before seal() and
...
def served_round(rd: Path, files: dict[str, bytes] | None = None, missed: list[str] | None = None):
    ...
    scar-1808: it serves `files`, bytes read once and held in memory. It used to read the disk
...
def _fresh_standard_entry(std_path: Path, page: Path, cur: str | None = None) -> dict | None:
    ...
    the bytes seal holds in memory (scar-1808), else the file as it stands."""
    cur = cur or sha(page)
...
def _run_producers(snap: RoundSnapshot, pages: list[Path], cfg: dict) -> list[tuple[str, list[str]]]:
    ...
    # both are handed over by path: the HELD copies (scar-1831), the bytes the chain reads too.
    ...
    # which seal overwrites and both skip, and a 9px page sealed COMPLETE (scar-1838)
    ...
    with served_round(rd, served_files(snap.files), snap.missed) as base:
        ...
        if (craft.get("require_impeccable") and craft.get("tier", "craft") != "wireframe"
            ...
            # file the census already holds and binds (round 2026-09-21, scar-1743)
...
def unbound_references(files: dict[str, bytes]) -> dict[str, list[str]]:
    ...
    site ships the round, so a visitor's fetch of standard.json would succeed (Sana, scar-1838).
...
SEAL_RESIDUAL = ("the same OS user can write to the snapshot directory mid-seal (the browser is served "
                 ...
                 "closes that window (scar-1834, which absorbed scar-1827). A checkout whose real path "
...
def _live(p: Path) -> Path:
    ...
    since scar-1831 every read during a seal is of held bytes."""
...
def find_config(start: Path) -> Path | None:
    ...
    symlink (scar-1811 review, findings 2 and 3)."""
...
def round_dir_for(page: Path) -> Path:
    ...
    complete fake round sitting there was what the chain checked (scar-1811 review, finding-1)."""
...
def craft_problems(rd: Path, page: Path, cfg: dict, cfg_path: Path | None) -> list[str]:
    ...
    whole from check_technique_parity.py, whose docstring carries the
    ...
    # to work from?" The original design-room had the step and only its back half was ported:
...
def round_lane(rd: Path) -> tuple[str, str | None]:
    ...
    # is what scar-1811 closed (caught by the full run for scar-1870)
...
# impeccable-sealed round read OPEN on every passive read after dc-04 added the stage (scar-1845)
...
def receipt_problems(page: Path) -> list[str]:
    ...
    left the round COMPLETE, because the digest seal wrote had no reader (scar-1808 finding-7).
...
def declared_sources(rd: Path, root: Path) -> dict[Path, str]:
    ...
    the reference site the served pages are built by the newest sealed round, which reads
...
# the reference site: a false claim in the hero also sat in
...
def corrected_page(page: Path) -> bool:
    ...
    if _held_snapshot(page) is not None:
        return False            # the passive gate's exemption alone, as sourced_page (scar-1831)
...
def reader_history(rd: Path, page: Path) -> dict:
    ...
    re-roll after a one-byte edit is refused past the cap and visible below it (scar-1840). A row with
    ...
    for k, line in enumerate(lines):
        ...
        # one per row made one old n=5 invocation five runs (scar-1840 std-2)
...
def reader_problems(rd: Path, page: Path, cfg: dict, cfg_path: Path | None = None) -> list[str]:
    ...
    # LEAVE rows unread (scar-1840 adv-1). Said once, by the first page of the round.
    ...
    if hist["runs"] > cap:
        # what works (scar-1840 std-1): editing the page never lowers a count taken over every version
        ...
        return [f"{page.name} has had {hist['runs']} reader runs in this round ({hist['leave_runs']} with a "
                ...
                f"recompute (scar-1834) runs the readers itself on the committed page."]
...
def _seal_snapshot(rd: Path, pages: list[Path], seal_cfg: dict, snap: RoundSnapshot) -> int:
    ...
    for p in pages:
        # scar-1811: the chain is checked against the SNAPSHOT's copy of the page, so every record
        # it reads is the one that was measured. It read the disk, and a brief swapped good for
        # the chain check and back before the final compare sealed (scar-1808 finding-10, NOT MET
    ...
    if isinstance(seal_cfg.get("readers"), dict):
        for p in pages:
            ...
            # the cap is still on the record (scar-1840)
...
def scan_roots(payload: dict) -> list[Path]:
    ...
    # (scar-1874). dc-17's decision is untouched: the 3-level descent below still finds a config
...
def citation_problems(rd: Path) -> list[str]:
    ...
    2026-09-18). The record is builder-writable like every round file; forging it is scar-1834's."""
...
def engine_problems(rd: Path) -> list[str]:
    ...
    the readers and the critique. The log is a round file, builder-writable like every one (scar-1834)."""
    try:
...
def record_brief_reads(rd: Path, transcript_path: str, session_id: str) -> None:
    ...
    still append to it or feed the hook by hand, which is scar-1834's (the CI reader run is the verdict
...
def brief_read_problems(rd: Path, cfg: dict, cfg_path: Path | None) -> list[str]:
    ...
    is builder-writable like every round file; forging it is scar-1834's."""
